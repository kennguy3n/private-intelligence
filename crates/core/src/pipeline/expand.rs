//! Bullet expansion pipeline.
//!
//! Expands bullet points into full prose using mT5-small with an
//! expand-specific LoRA adapter.
...
/// Run a bullet expansion task.
///
/// Input: bullet points.
/// Output: full prose expanding the bullets.
pub async fn run(
    engine: &mut AiEngine,
    bullets: &str,
    language: &str,
    ...
    let adapter_path = engine.resolve_adapter_path(
        &format!("adapters/expand.{}.bin", language)
    );
    let adapter = LoRAAdapter::new("expand", language, &adapter_path, 8);
    ...
    if options.stream {
        ...
        return collect_stream_result(rx, Task::Expand, "mt5-small-int8", Some(adapter_id), start).await;
    }
