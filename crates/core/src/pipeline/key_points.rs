...
pub async fn run(
    ...
    let adapter_path = engine.resolve_adapter_path(
        &format!("adapters/keypoints.{}.bin", language)
    );
    let adapter = LoRAAdapter::new("keypoints", language, &adapter_path, 8);
    ...
    if options.stream {
        ...
        return collect_stream_result(rx, Task::KeyPoints, "mt5-small-int8", Some(adapter_id), start).await;
    }
