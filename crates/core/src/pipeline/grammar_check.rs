//! Grammar and spelling check pipeline.
//!
//! Proofreads text and returns corrections with inline diff markers
//! using mT5-small with a grammar-specific LoRA adapter.
...
/// Run a grammar check task.
///
/// Input: text to proofread.
/// Output: corrected text with inline diff markers.
pub async fn run(
    engine: &mut AiEngine,
    text: &str,
    language: &str,
    ...
    let adapter_path = engine.resolve_adapter_path(
        &format!("adapters/grammar.{}.bin", language)
    );
    let adapter = LoRAAdapter::new("grammar", language, &adapter_path, 8);
    ...
    if options.stream {
        ...
        return collect_stream_result(rx, Task::GrammarCheck, "mt5-small-int8", Some(adapter_id), start).await;
    }
